...
class FileValidationMiddleware:
                ...
                try:
                    ...
                    response = JSONResponse(
                        ...
                        content={
                            ...
                            "message": f"Unable to parse uploaded files: {str(e)}"
                        }
                    )
                    await response(scope, receive, send)
                    return
    ...
    def _should_validate_file(self, request: Request) -> bool:
        ...
        return method in ['POST', 'PUT'] and any(path.startswith(upload_path) for upload_path in file_upload_paths)
    ...
    async def _validate_single_file(self, file: UploadFile) -> Dict[str, Any]:
        ...
        if file.content_type and file.content_type not in self.ALLOWED_MIME_TYPES:
            issues.append(f'Unsupported MIME type: {file.content_type}')
        
        # 4. 文件大小检测
        file_size = await self._get_file_size(file)
        if file_size > self.MAX_FILE_SIZE:
            issues.append(f'File too large: {file_size} bytes (max: {self.MAX_FILE_SIZE} bytes)')
        if file_size < self.MIN_FILE_SIZE:
            issues.append(f'File too small: {file_size} bytes (min: {self.MIN_FILE_SIZE} bytes)')
...
class AuthMiddleware:
    ...
    async def _validate_token(self, token: str) -> Optional[str]:
        ...
        try:
            ...
            # 记录错误但不暴露给客户端
            print(f"Token validation error: {str(e)}")  # 正式环境中应使用正式的日志系统
            return None
