...
async def validate_user_id_in_minio(user_id: str) -> bool:
    ...
    minio_client = Minio(
        minio_config['address'] if 'address' in minio_config else f"{minio_config['host']}:{minio_config['port']}",
...
def convert_to_internal_minio_url(url: str) -> str:
    ...
    # 处理新的address格式或旧的host/port格式
    address = minio_cfg.get("address")
    if address:
        internal_prefix = f"http://{address}"
    else:
        ...
        if not host or not port:
            # 配置不完整，返回原 URL
            return url
        internal_prefix = f"http://{host}:{port}"
    ...
    internal_url = f"{internal_prefix}{path}"
    return internal_url
