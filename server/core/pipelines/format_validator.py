...
from ..utils.tools import read_config, detect_content_type
...
class FileFormatValidator:
    ...
    async def validate_file_content(self, file_content: bytes, filename: str) -> Dict[str, Any]:
        """验证文件内容"""
        validation_result = {
            "is_valid": False,
        ...
        # 检查文件大小限制（可配置）
        max_file_size = self.config.get("file_limits", {}).get("max_file_size", 100 * 1024 * 1024)  # 默认100MB
        if len(file_content) > max_file_size:
            validation_result["errors"].append(f"File size ({len(file_content)} bytes) exceeds limit ({max_file_size} bytes)")
            return validation_result
        ...
        logger.info(f"File content validated: {filename}, size: {len(file_content)} bytes")
...
class PDFConverter:
    ...
    def _fix_public_url(self, original_url: str) -> str:
        """修正公网URL"""
        try:
            public_prefix = self.config["server_components"]["minio"].get("public_url_prefix")
    ...
    async def convert_document_to_pdf(self, file_url: str) -> Optional[str]:
        ...
        try:
            ...
            if file_url.lower().endswith(".pdf"):
                logger.info(f"File is already PDF format, no conversion needed: {file_url}")
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                if "converted_url" not in result or not result["converted_url"]:
                    logger.error(f"Conversion service returned invalid result: {result}")
                    return None
                
                converted_url = result["converted_url"]
                logger.info(f"Document conversion successful: {file_url} -> {converted_url}")
                converted_url = self._fix_public_url(converted_url)
                return converted_url
        
        except httpx.HTTPStatusError as e:
            logger.error(f"Conversion service HTTP error: {e.response.status_code} - {e.response.reason_phrase}")
...
class FormatValidationPipeline:
    ...
    async def process_file(self, filename: str, file_content: bytes, file_url: str) -> Dict[str, Any]:
        """
        处理文件：验证格式并转换为PDF
        ...
        try:
            ...
            if not self.validator.validate_file_type(filename):
                result["errors"].append(f"Unsupported file type: {Path(filename).suffix}")
                return result
            
            validation_result = await self.validator.validate_file_content(file_content, filename)
            ...
            result["success"] = True
            logger.info(f"Format validation and PDF conversion pipeline completed for {filename}")
