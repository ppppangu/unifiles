...
from typing import Optional, List, Dict, Any, Tuple, Protocol, Union
from loguru import logger
from tenacity import retry, stop_after_attempt, wait_exponential, retry_if_exception_type

from ..utils.tools import read_config, convert_to_internal_minio_url
    ...
    async def extract_text_from_pdf(self, pdf_path_or_url: str) -> str:
        ...
        try:
            ...
            if pdf_path_or_url.startswith(('http://', 'https://')):
                # 这里应该调用下载逻辑，简化处理
                logger.info(f"URL processing not implemented in SimplePDFReader: {pdf_path_or_url}")
                return "URL processing not supported in SimplePDFReader"
            
            # 获取页数
            page_count = await asyncio.to_thread(self._get_pdf_page_count, pdf_path_or_url)
            if page_count == 0:
                return ""
...
class MineruOCRProvider:
    ...
    async def request_mineru_processing(self, file_url: str) -> Optional[str]:
        ...
        try:
            ...
            mineru_endpoint = self.config.get("mineru", {}).get("endpoint", "http://localhost:8080")
    ...
    async def extract_markdown_from_pdf(self, pdf_path_or_url: str) -> str:
        ...
        return await self.extract_text_from_pdf(pdf_path_or_url)  # Mineru直接返回Markdown
    ...
    async def download_file(self, file_url: str, file_path: str) -> str:
        ...
        async def _do_request(target_url: str):
            ...
            try:
                ...
                limits = httpx.Limits(max_keepalive_connections=50, max_connections=300, keepalive_expiry=30.0)
                ...
                async with httpx.AsyncClient(
                    ...
                    try:
                        ...
                        logger.info(f"HEAD request success: {head_response.status_code}, Content-Length: {head_response.headers.get('content-length', 'unknown')}")
                    except Exception as head_error:
                        logger.warning(f"HEAD request failed, continuing with GET: {head_error}")
                    ...
                    if not response.content:
                        ...
                        raise ValueError(f"Downloaded file content is empty: {target_url}")
                    ...
                    logger.info(f"File download successful: {file_path}, size: {len(response.content)} bytes")
                    return file_path
        ...
        try:
            ...
            if internal_url != original_url:
                logger.warning(f"First download failed, trying internal URL: {internal_url}")
                try:
...
class TextProcessor:
    ...
    def find_all_text_and_image_index(self, text: str, user_id: str, knowledge_base_id: str, 
                                    document_id: str) -> Tuple[str, List[Tuple[int, int, str]]]:
        """
    ...
    async def split_text(self, text: str, mode: str = "SlidingWindow", module: str = "normal") -> List[str]:
        """根据指定的模式和模块对文本进行分块"""
        if mode == "SlidingWindow" and module == "normal":
            chunk_size = self.config.get("text_chunk_strategy", {}).get("SlidingWindow", {}).get("chunk_size", 1000)
            chunk_overlap = self.config.get("text_chunk_strategy", {}).get("SlidingWindow", {}).get("chunk_overlap", 200)
    ...
    def save_results_to_json_sync(self, converted_text: str, results: List[Tuple[int, int, str]]) -> List[Dict[str, Any]]:
        """将结果转换为字典列表"""
        results_dict = []
    ...
    async def process_text_content(self, text: str, user_id: str, knowledge_base_id: str, 
        ...
        try:
            ...
            converted_text, results = await asyncio.to_thread(
                self.find_all_text_and_image_index, text, user_id, knowledge_base_id, document_id
            )
            logger.info(f"Image boundary segmentation completed, {len(results)} segments created")
            
            # 转换为字典格式
            results_dict = await asyncio.to_thread(self.save_results_to_json_sync, converted_text, results)
            ...
            logger.info(f"Text chunking completed, final {len(new_results_dict)} segments")
...
class PDFProcessingPipeline:
    ...
    def __init__(self, ocr_provider: Optional[OCRProvider] = None, config: Optional[Dict[str, Any]] = None):
        self.config = config or read_config()
    ...
    async def process_pdf_simple(self, pdf_path: str) -> str:
        ...
        try:
            ...
            logger.info(f"Simple mode PDF processing completed, {len(text)} characters extracted")
            return text
    ...
    async def process_pdf_normal(self, pdf_url: str) -> str:
        ...
        try:
            ...
            if not text or text.strip() == "":
                ...
                logger.info("OCR processing returned empty result, using placeholder text")
            
            logger.info(f"Normal mode PDF processing completed, {len(text)} characters extracted")
            return text
    ...
    async def process_pdf_to_structured_content(self, pdf_url: str, user_id: str, 
                                              knowledge_base_id: str, document_id: str, 
                                              mode: str = "simple") -> List[Dict[str, Any]]:
        """
        处理PDF到结构化内容
        ...
        try:
            ...
            logger.info(f"Request: user_id={user_id}, file_url={pdf_url}, knowledge_base_id={knowledge_base_id}, mode={mode}")
            ...
            logger.info(f"Content structure processing completed, {len(structured_content)} segments")
