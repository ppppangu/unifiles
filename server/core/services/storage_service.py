...
from ..utils.tools import read_config, read_minio_config, detect_content_type
from ..database.manager import DatabaseManager
from ..database.models import ChunkModel, PhotoModel, DocumentModel
...
class MinIOStorageManager:
    ...
    def generate_public_url(self, object_path: str) -> str:
        ...
        if self.minio_config.get("use_public_url", False) and self.minio_config.get("public_url_prefix"):
            return f"{self.minio_config['public_url_prefix']}/{bucket_name}/{object_path}"
        else:
            # 使用内网地址
            if "address" in self.minio_config:
    ...
    async def upload_file(self, object_path: str, file_content: bytes, content_type: str = None) -> str:
        """
        上传文件到MinIO
    ...
    async def upload_file_from_path(self, object_path: str, local_file_path: str, content_type: str = None) -> str:
        """
        从本地文件路径上传到MinIO
        ...
        try:
            ...
            public_url = self.generate_public_url(object_path)
            logger.info(f"File uploaded from path to MinIO: {local_file_path} -> {object_path}, URL: {public_url}")
...
class VectorStorageManager:
    ...
    async def save_text_chunk(self, chunk_id: str, document_id: str, text: str, 
                            doc_position: int, embedding: List[float]) -> ChunkModel:
        """保存文本块到向量数据库"""
        try:
    ...
    async def save_image_data(self, photo_id: str, document_id: str, image_text: str, 
                            doc_position: int, embedding: List[float], 
                            photo_type: str = "photo", base64_image: str = None) -> PhotoModel:
        """保存图片数据到向量数据库"""
        try:
    ...
    async def batch_save_structured_content(self, user_id: str, knowledge_base_id: str, 
                                          document_id: str, document_name: str, 
                                          content_list: List[Dict[str, Any]]) -> List[str]:
        """
        批量保存结构化内容到向量数据库
        ...
        try:
            ...
            await self.db_manager.ensure_knowledge_base_exists(knowledge_base_id, user_id, f"Knowledge Base {knowledge_base_id}")
            await self.db_manager.ensure_document_exists(document_id, knowledge_base_id, document_name)
            ...
            logger.info(f"Batch saved {len(saved_ids)} structured content items to vector database")
            return saved_ids
...
class StorageService:
    ...
    async def store_processed_files(self, user_id: str, knowledge_base_id: str, document_id: str,
                                  markdown_content: str, pdf_file_path: str, 
        ...
        try:
            ...
            logger.info(f"Processed files stored successfully - MD: {md_url}, PDF: {pdf_url}")
            
            return {
                "markdown_public_url": md_url,
                "pdf_file_public_url": pdf_url
            }
    ...
    async def store_structured_content_to_vector_db(self, user_id: str, knowledge_base_id: str,
                                                  document_id: str, document_name: str,
                                                  structured_content: List[Dict[str, Any]]) -> List[str]:
        """将结构化内容存储到向量数据库"""
    ...
    async def save_processing_results(self, user_id: str, knowledge_base_id: str, document_id: str,
                                    document_name: str, markdown_content: str, pdf_file_path: str,
                                    structured_content: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        保存完整的处理结果
        ...
        try:
            ...
            store_files_task = self.store_processed_files(
                user_id, knowledge_base_id, document_id, markdown_content, pdf_file_path, document_name
            )
            ...
            # 等待所有任务完成
            file_urls, component_ids = await asyncio.gather(store_files_task, store_vector_task)
            ...
            logger.info(f"Processing results saved successfully: {len(structured_content)} components stored")
            return result
    ...
    async def cleanup_temporary_files(self, *file_paths: str):
        ...
        for file_path in file_paths:
            if file_path and Path(file_path).exists():
                cleanup_tasks.append(asyncio.to_thread(Path(file_path).unlink, missing_ok=True))
    ...
    def get_service_info(self) -> Dict[str, Any]:
        """获取服务信息"""
        return {
            "minio_endpoint": self.minio_manager.minio_config.get("address") or f"{self.minio_manager.minio_config['host']}:{self.minio_manager.minio_config['port']}",
