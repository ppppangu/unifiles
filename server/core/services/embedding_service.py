...
class SingletonEmbeddingProvider:
    ...
    def _get_latest_embedding_instance(self, instance_type: str = None, alias: str = None):
        ...
        if alias:
            ...
            if alias == "bge-m3":
                ...
                return "text-embedding-ada-002", "https://api.openai.com/v1/embeddings", "your-openai-key", "text-embedding-ada-002"
        
        if instance_type == "language_embedding":
            return "bge-m3", "http://localhost:8080/v1/embeddings", "your-api-key", "bge-m3"
        elif instance_type == "multimodal_llm":
            return "gpt-4-vision", "http://localhost:8081/v1/chat/completions", "your-api-key", "gpt-4-vision"
    ...
    async def embed_text(self, text: str, alias: str = "bge-m3") -> List[float]:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=timeout, limits=limits, http2=False, verify=False) as client:
                ...
                if "data" in result and len(result["data"]) > 0 and "embedding" in result["data"][0]:
                    ...
                    logger.debug(f"Text embedding successful, dimension: {len(embedding)}")
                    return embedding
                else:
    ...
    async def describe_image(self, image_url: str, alias: str = "") -> str:
        ...
        try:
            if not alias:
                name, url, key, alias = self._get_latest_embedding_instance(instance_type="multimodal_llm")
            else:
            ...
            data = {
                ...
                "messages": [
                    ...
                    {"role": "user", "content": [{"type": "text", "text": "请详细描述这张图片的内容"}]}
                ],
                "stream": False
            }
            ...
            async with httpx.AsyncClient(timeout=timeout, limits=limits, http2=False, verify=False) as client:
                ...
                if "choices" in result and len(result["choices"]) > 0:
                    ...
                    logger.debug(f"Image description successful, length: {len(description)}")
                    return description
                else:
    ...
    async def embed_image_description(self, image_url: str, alias: str = "bge-m3") -> Tuple[str, List[float]]:
        """嵌入图片：先描述图片，再对描述进行嵌入"""
        try:
            # 先获取图片描述
            ...
            logger.debug(f"Image embedding successful: description length={len(description)}, embedding dimension={len(embedding)}")
...
class EmbeddingService:
    ...
    def __init__(self, provider: Optional[EmbeddingProvider] = None, config: Optional[Dict[str, Any]] = None):
        self.config = config or read_config()
