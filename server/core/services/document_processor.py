...
from ..pipelines.pdf_processor import PDFProcessingPipeline, SimplePDFReader, MineruOCRProvider
...
class DocumentProcessingService:
    ...
    async def process_file_from_upload(self, filename: str, file_content: bytes, 
                                     user_id: str, knowledge_base_id: str = None,
        ...
        try:
            ...
            # 格式验证和PDF转换
            validation_result = await self.format_pipeline.process_file(filename, file_content, file_url)
            ...
            logger.info(f"Content extraction completed: {len(structured_content)} segments")
            
            # 第三步：嵌入处理
            logger.info(f"=== Stage 3: Embedding processing ===")
            
            embedded_content = await self.embedding_service.embed_content_batch(structured_content)
            logger.info(f"Embedding processing completed for {len(embedded_content)} segments")
            ...
            logger.info(f"Document ID: {document_id}, Components: {storage_result['component_count']}")
    ...
    async def process_file_from_url(self, file_url: str, user_id: str, 
                                  knowledge_base_id: str = None, mode: str = "simple",
                                  raw_file_url_to_return: str = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            validation_result = await self.format_pipeline.process_file_url_only(file_url)
            ...
            component_ids = await self.storage_service.store_structured_content_to_vector_db(
                user_id=user_id,
                knowledge_base_id=knowledge_base_id,
                document_id=document_id,
                document_name=filename,
            ...
            logger.info(f"Vector database storage completed: {len(component_ids)} components stored")
            ...
            # 清理临时文件
            await self.storage_service.cleanup_temporary_files(str(temp_md_file), str(temp_pdf_file))
...
def get_document_processor(config: Optional[Dict[str, Any]] = None) -> DocumentProcessingService:
    """获取默认文档处理服务实例（单例模式）"""
...
async def mineru_process(file_url: str, knowledge_base_id: str, mode: str, user_id: str,
                        raw_file_url_to_return: str = "") -> Optional[Dict[str, str]]:
    """
