...
class DatabaseManager:
    ...
    async def create_knowledge_base(self, kb_model: KnowledgeBaseModel) -> KnowledgeBaseModel:
        """创建知识库"""
        conn = await self.get_connection()
        try:
            async with conn.transaction():
    ...
    async def ensure_knowledge_base_exists(self, kb_id: str, user_id: str, name: str = None) -> KnowledgeBaseModel:
        """确保知识库存在，不存在则创建"""
    ...
    async def create_document(self, doc_model: DocumentModel) -> DocumentModel:
        ...
        try:
            async with conn.transaction():
                ...
                # 获取创建的文档信息
                result = await conn.fetchrow(
                    "SELECT * FROM chunk_schema.documents WHERE id = $1",
                    doc_model.id
                )
    ...
    async def ensure_document_exists(self, doc_id: str, kb_id: str, name: str = None) -> DocumentModel:
        """确保文档存在，不存在则创建"""
    ...
    async def update_document_urls(self, doc_id: str, markdown_url: str, raw_file_url: str) -> bool:
        """更新文档URL信息"""
        conn = await self.get_connection()
        try:
            async with conn.transaction():
                await conn.execute(
                    ...
                    WHERE id = $1
                    """,
                    doc_id, markdown_url, raw_file_url
                )
    ...
    async def save_chunk(self, chunk_model: ChunkModel) -> ChunkModel:
        ...
        try:
            async with conn.transaction():
                ...
                logger.info(f"Chunk saved: {chunk_model.id} at position {chunk_model.doc_position}")
                return chunk_model
    ...
    async def save_photo(self, photo_model: PhotoModel) -> PhotoModel:
        ...
        try:
            async with conn.transaction():
                ...
                logger.info(f"Photo saved: {photo_model.id} at position {photo_model.doc_position}")
                return photo_model
    ...
    async def create_processing_log(self, log_model: ProcessingLogModel) -> ProcessingLogModel:
        """创建处理日志"""
        conn = await self.get_connection()
        try:
            async with conn.transaction():
    ...
    async def update_processing_log(self, log_id: str, status: ProcessingStatus, 
                                  message: str = None, error_info: Dict[str, Any] = None) -> bool:
        """更新处理日志状态"""
        conn = await self.get_connection()
        try:
            async with conn.transaction():
                completed_at = datetime.now() if status in [ProcessingStatus.COMPLETED, ProcessingStatus.FAILED] else None
