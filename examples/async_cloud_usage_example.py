...
async def cloud_example():
    ...
    # 2. 创建知识库
    knowledge_base = await client.create_knowledge_base(
        name="我的云端知识库", description="存储在云端的重要文档"
    )
    ...
    # 4. 删除知识库（已注释，生产环境谨慎使用）
    # await client.delete_knowledge_base("old_kb_id")

    # 5. 上传文档
    example_kb = kb_list[0]
    document = await example_kb.upload_document(file_path="/path/to/your/document.pdf")
    ...
    # 7. 重新获取知识库列表，查看已上传的文档
    updated_kb_list = await example_kb.list_document(
        knowledge_base_id=knowledge_base.id
    )
    print("✅ 文档处理完成，更新后的知识库列表:")
    for kb in updated_kb_list:
        print(f"   - {kb.name} (文档数: {kb.document_count})")
