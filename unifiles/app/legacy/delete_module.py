    ...
    try:
        ...
        try:
            ...
            async with conn.transaction():
                ...
                if not kb_result:
                    raise ValueError(
                        f"Knowledge base {knowledge_base_id} does not belong to user {user_id} or does not exist"
                    )
                ...
                doc_result = await conn.fetchrow(
                    doc_check_query, file_id, knowledge_base_id
                )

                if not doc_result:
                    raise ValueError(
                        f"Document {file_id} does not belong to knowledge base {knowledge_base_id} or does not exist"
                    )
                ...
                if result == "DELETE 0":
                    logger.warning(
                        f"No document found to delete - document_id: {file_id}, knowledge_base_id: {knowledge_base_id}"
                    )
                    return False

                # 记录成功删除的详细信息
                logger.info(
                    f"Successfully deleted document from vector database - "
                ...
                # 记录触发器自动处理的操作
                logger.info(
                    f"Database triggers automatically handled: "
        ...
        raise ValueError(
            f"Cannot delete document due to foreign key constraints: {str(e)}"
        )
    ...
    try:
        ...
        try:
            objects = minio_client.list_objects(
                bucket_name, prefix=primary_path, recursive=True
            )
            primary_files = list(objects)

            if primary_files:
                logger.info(
                    f"Found {len(primary_files)} files in primary path: {primary_path}"
                )
                ...
                logger.info(
                    f"Successfully deleted {len(deleted_files)} files from primary path"
                )
            else:
            ...
            logger.error(
                f"Error listing/deleting files from primary path {primary_path}: {e}"
            )
        ...
        if not deleted_files:
            try:
                objects = minio_client.list_objects(
                    bucket_name, prefix=default_path, recursive=True
                )
                default_files = list(objects)

                if default_files:
                    logger.info(
                        f"Found {len(default_files)} files in default path: {default_path}"
                    )
                    ...
                    for obj in default_files:
                        try:
                            ...
                            logger.error(
                                f"Failed to delete file {obj.object_name}: {e}"
                            )

                    logger.info(
                        f"Successfully deleted {len(deleted_files)} files from default path"
                    )
                else:
                ...
                logger.error(
                    f"Error listing/deleting files from default path {default_path}: {e}"
                )

        # 检查删除结果
        if deleted_files:
            logger.info(
                f"Successfully deleted file from MinIO - "
            ...
            logger.warning(
                f"No files found to delete for file_id: {file_id}, "
