...
# 合并所有实例
_all_instances = (
    readed_language_embeddings_list
    + readed_multimodal_llm_list
    + readed_language_llm_list
)
...
def get_latest_embedding_instance(
    alias: Optional[str] = None, instance_type: Optional[str] = None
):
    """
    获取最新索引位置的embedding实例
    ...
    if alias is not None and instance_type is not None:
        raise ValueError(
            "alias and instance_type parameters are mutually exclusive. Please specify only one."
        )
    ...
    if alias:
        ...
        if not type_instances:
            raise ValueError(
                f"No instances found for type: {instance_type}. Supported types: {list(_type_instances.keys())}"
            )
