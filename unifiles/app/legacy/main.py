...
supported_file_types = (
    document_file_types + pdf_file_types + [".py", ".ipynb", ".js", ".json"]
)
...
async def get_supported_file_types():
    return {
        ...
        "data": {"supported_file_types": supported_file_types},
    }
...
async def upload_minio(
    ...
    try:
        ...
        # 生成公网URL
        if minio_config.get("use_public_url", False) and minio_config.get(
            "public_url_prefix"
        ):
            ...
            public_url = f"http://{minio_config['address']}/{bucket_name}/{user_id}/default_file_space/{file_uuid}/{filename}"
...
async def convert_document_to_pdf(file_url: str):
    try:
        ...
        convert_url = config["convert_format_server"][0]["url"] + "/convert"
