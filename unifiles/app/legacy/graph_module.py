...
from server.core.utils.tools import read_config, read_pg_config
...
async def extract_summary(
    ...
    logger.info(
        f"Starting extract_summary - text_length: {len(text)}, has_kb_structure: {bool(knowledge_base_structure)}, has_user_info: {bool(user_info)}, document_location: {document_location}"
    )

    prompt = extract_info_template.render(
    ...
    # 获取语言模型实例
    try:
        language_llm_instance = get_latest_embedding_instance(
            instance_type="language_llm"
        )
        # 获取语言模型实例的name,url,key,alias
        language_llm_name, language_llm_url, language_llm_key, language_llm_alias = (
            language_llm_instance
        )
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        if response.status_code == 200:
            ...
            logger.error(
                f"Language model API error: {response.status_code}, response: {response.text}"
            )
...
async def single_document_summary(
    ...
    logger.debug(
        f"Starting single_document_summary - text_length: {len(text) if text else 0}, window_size: {window_size}, document_location: {document_location}"
    )
...
async def database_validate(user_id: str, knowledge_base_id: str) -> list[dict]:
    ...
    logger.info(
        f"Starting database_validate - user_id: {user_id}, knowledge_base_id: {knowledge_base_id}"
    )
    ...
    try:
        ...
        async with conn.transaction():
            ...
            logger.info(
                f"User validation result: {len(user_exist_result)} records found"
            )
            ...
            logger.debug(
                f"Executing knowledge base validation query: {knowledge_base_exist_query}"
            )
            knowledge_base_exist_result = await conn.fetch(knowledge_base_exist_query)
            logger.info(
                f"Knowledge base validation result: {len(knowledge_base_exist_result)} records found"
            )

            if len(knowledge_base_exist_result) == 0:
                logger.warning(
                    f"Knowledge base {knowledge_base_id} not found in chunk_schema.knowledge_bases table"
                )
            ...
            logger.info(
                f"Document list result: {len(document_list_result)} documents found"
            )

            if len(document_list_result) == 0:
                logger.warning(
                    f"No documents found in knowledge base {knowledge_base_id}"
                )
                return {
                    "status": "error",
            ...
            logger.info(
                f"Database validation successful - user: {user_id}, kb: {knowledge_base_id}, docs: {len(document_list_result)}"
            )
...
async def produce_summary_task(user_id: str, knowledge_base_id: str, document_id: str):
    ...
    logger.warning(
        f"produce_summary_task called with deprecated function - user_id: {user_id}, kb: {knowledge_base_id}, doc: {document_id}"
    )
    try:
        # 这里只是占位实现, 实际业务请使用produce_document_graph
        return {
            "status": "ok",
            "message": "Task is deprecated, please use produce_document_graph",
        }
    except Exception as e:
...
async def produce_document_graph(user_id: str, knowledge_base_id: str):
    ...
    logger.info(
        f"Starting produce_document_graph - user_id: {user_id}, knowledge_base_id: {knowledge_base_id}"
    )
    ...
    try:
        ...
        async with conn.transaction():
            ...
            # 校验知识库
            logger.debug(
                f"Validating knowledge base: {knowledge_base_id} for user: {user_id}"
            )
            ...
            if not kb_row:
                logger.warning(
                    f"Knowledge base {knowledge_base_id} not found for user {user_id}"
                )
                return {"status": "error", "message": "Knowledge base not found"}
            ...
            logger.info(
                f"Found {len(documents)} documents in knowledge base {knowledge_base_id}"
            )
            ...
            # 知识库整体结构
            logger.debug(
                f"Fetching hierarchy structure for knowledge base: {knowledge_base_id}"
            )
            ...
            overall_structure = (
                str(hierarchy_row["labels"])
                if hierarchy_row and hierarchy_row["labels"]
                else ""
            )
            logger.debug(
                f"Knowledge base structure: {overall_structure[:100]}..."
            ...
            # 并发生成 tags
            logger.info(
                f"Starting concurrent tag generation for {len(documents)} documents"
            )
            tasks = []
            for i, doc in enumerate(documents):
                ...
                tasks.append(
                    single_document_summary(
                        ...
                        user_info=user_info,
                        document_location=(
                            str(doc["hierarchy_path"]) if doc["hierarchy_path"] else ""
                        ),
            ...
            for doc, tags in zip(documents, tags_list):
                if tags:
                    logger.debug(
                        f"Updating tags for document {doc['id']}: {tags} (type: {type(tags)})"
                    )
                    await conn.execute(
                    ...
                    logger.debug(
                        f"Verification read for document {doc['id']}: {verification['tags']} (type: {type(verification['tags'])})"
                    )
                else:
                    logger.debug(f"No tags generated for document {doc['id']}")

            logger.info(
                f"Successfully updated tags for {updated_count}/{len(documents)} documents"
            )
            return {"status": "ok", "message": "Tags generated successfully"}
    except Exception as e:
        logger.error(
            f"produce_document_graph error for user {user_id}, kb {knowledge_base_id}: {e}",
            exc_info=True,
        )
        return {"status": "error", "message": str(e)}
    finally:
...
async def get_documents_graph(user_id: str, knowledge_base_id: str):
    ...
    logger.info(
        f"Starting get_documents_graph - user_id: {user_id}, knowledge_base_id: {knowledge_base_id}"
    )
    ...
    try:
        ...
        async with conn.transaction():
            ...
            logger.info(
                f"Found {len(rows)} documents in knowledge base {knowledge_base_id}"
            )
            ...
            for i, r in enumerate(rows):
                ...
                if raw_tags is None:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Failed to parse tags as JSON for document {r['id']}: {raw_tags}"
                        )
                        tags = []
                else:
                    # 其他类型，尝试转换为列表
                    try:
                        ...
                        logger.warning(
                            f"Unexpected tags type for document {r['id']}: {type(raw_tags)} - {raw_tags}"
                        )
                        tags = []
            ...
            logger.info(
                f"Successfully retrieved {len(documents)} documents for knowledge base {knowledge_base_id}"
            )
            ...
            for doc in documents:
                if doc["tags"]:
                    logger.info(
                        f"Document {doc['id']} has {len(doc['tags'])} tags: {doc['tags']}"
                    )
                else:
        ...
        logger.error(
            f"get_documents_graph error for user {user_id}, kb {knowledge_base_id}: {e}",
            exc_info=True,
        )
        return {"status": "error", "message": str(e)}
    finally:
...
async def produce_subject_graph(user_id: str, knowledge_base_id: str):
    logger.info(
        f"produce_subject_graph called - user_id: {user_id}, knowledge_base_id: {knowledge_base_id}"
    )
...
async def get_subject_graph(user_id: str, knowledge_base_id: str):
    logger.info(
        f"get_subject_graph called - user_id: {user_id}, knowledge_base_id: {knowledge_base_id}"
    )
