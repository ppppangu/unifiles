...
from server.core.utils.tools import convert_to_internal_minio_url, detect_content_type

from .singleton_embedding import get_latest_embedding_instance

with open(Path(__file__).parent.parent.parent.parent / "config.yaml", "r") as f:
...
async def plumber_read_pdf(file_url: str):
    ...
    # 读取pdf文件
    tasks = [
        asyncio.to_thread(plumber_read_pdf_page, file_url, page_num)
...
async def describe_photo(photo_url: str, index: int, alias: str = ""):
        ...
        "messages": [
            {
                "role": "user",
                "content": [
                    {"type": "image_url", "image_url": {"url": f"{photo_url}"}}
                ],
            },
            {
                "role": "user",
                "content": [{"type": "text", "text": "请详细这张图片的内容"}],
            },
        ],
        "stream": False,
    }
    ...
    try:
        ...
        logger.error(
            f"图片描述请求失败: {str(e)}, URL: {url}, 错误类型: {type(e).__name__}"
        )
        raise
...
async def save_results_to_json(converted_text: str, results: list, file_path: str):
    ...
    results_dict = await asyncio.to_thread(
        save_results_to_json_sync, converted_text, results
    )
    ...
    # 分配任务
    tasks = [
        describe_photo(extract_photo_url(result["content"]), index)
        for index, result in enumerate(results_dict)
        if result["type"] == "image"
    ]
    results = await asyncio.gather(*tasks)
    for index, result in results:
        results_dict[index]["content"] = replace_photo_text(
            results_dict[index]["content"], result
        )
    # 将results_dict存成json文件
    async with aiofiles.open(file_path, "w", encoding="utf-8") as f:
        await f.write(json.dumps(results_dict, ensure_ascii=False, indent=2))
    logger.info(
        f"保存results_dict到json文件成功，共{len(results_dict)}个条目，文件路径: {file_path}"
    )
...
async def split_text(text: str, mode: str, module: str):
    ...
    if mode == "SlidingWindow":
        if module == "normal":
            ...
            chunk_overlap = config["text_chunk_strategy"]["SlidingWindow"][
                "chunk_overlap"
            ]
...
async def split_text_by_json(json_file_path: str):
    ...
    for item in results_dict:
        if item["type"] == "text":
            ...
            for chunk in text_chunks:
                new_results_dict_list.append(
                    {
                        "content": chunk,
                        "index": len(new_results_dict_list),
                        "type": "text",
                        "embedding": None,
                    }
                )
    ...
    logger.info(
        f"将json文件中的文本进行分块策略成功,覆写原文件，共{len(new_results_dict_list)}个条目，文件路径: {json_file_path}"
    )
    ...
    try:
        ...
        try:
            async with conn.transaction():
                ...
                """
                kb_result = await conn.fetchrow(
                    kb_check_query, knowledge_base_id, user_id
                )

                if not kb_result:
                    logger.info(
                        f"Knowledge base {knowledge_base_id} not found for user {user_id}, creating new knowledge base"
                    )
                    kb_insert_query = """
                    ...
                    logger.info(
                        f"Successfully created knowledge base {knowledge_base_id}"
                    )
                ...
                doc_result = await conn.fetchrow(
                    doc_check_query, document_id, knowledge_base_id, user_id
                )

                if not doc_result:
                    logger.info(
                        f"Document {document_id} not found in knowledge base {knowledge_base_id}, creating new document"
                    )
                    doc_insert_query = """
                ...
                if embedding is not None:
                    ...
                    logger.debug(
                        f"Converted embedding to pgvector format: {embedding_vector[:100]}... (length: {len(embedding)})"
                    )
                ...
                logger.info(
                    f"Successfully saved text chunk to database - "
        ...
        try:
            async with conn.transaction():
                ...
                if embedding is not None:
                    ...
                    embedding_vector = f"[{','.join(map(str, embedding))}]"
                    logger.debug(
                        f"Converted photo embedding to pgvector format: {embedding_vector[:100]}... (length: {len(embedding)})"
                    )
                ...
                logger.info(
                    f"Successfully saved photo to database - "
...
async def save_something_to_vcdb(
    user_id: str,
    knowledge_base_id: str,
    document_id: str,
    document_name: str,
    ...
    if type == "text":
        return await save_text_to_vcdb_text_table(
            text,
            document_id,
            document_name,
            user_id,
        ...
        return await save_photo_to_vcdb_photo_table(
            photo_url=photo_url,
            user_id=user_id,
...
async def embedding_json_file(file_path: str, alias: str = "bge-m3"):
    ...
    for index, embedding, text, type in results:
        new_results_dict.append(
            {"index": index, "content": text, "type": type, "embedding": embedding}
        )
    async with aiofiles.open(file_path, "w", encoding="utf-8") as f:
        await f.write(json.dumps(new_results_dict, ensure_ascii=False, indent=2))
    logger.info(
        f"将json文件中的全部类目全部条目embedding做嵌入完成，共{len(new_results_dict)}个条目，文件路径: {file_path}"
    )
    # 返回文件路径
    return file_path
...
async def embedding_text(
    ...
    try:
        ...
        logger.error(
            f"Embedding请求失败: {str(e)}, URL: {url}, 错误类型: {type(e).__name__}"
        )
        raise
...
async def download_file(file_url: str, file_path: str):
    ...
    async def _do_request(target_url: str):
        ...
        try:
            ...
            # 配置更宽松的限制
            limits = httpx.Limits(
                max_keepalive_connections=50, max_connections=300, keepalive_expiry=30.0
            )
            ...
            async with httpx.AsyncClient(
                ...
                try:
                    ...
                    logger.info(
                        f"HEAD请求成功: {head_response.status_code}, Content-Length: {head_response.headers.get('content-length', 'unknown')}"
                    )
                ...
                logger.info(
                    f"文件下载成功: {file_path}, 大小: {len(response.content)} 字节"
                )
                return file_path
    ...
    try:
        ...
        if internal_url != original_url:
            logger.warning(
                f"首次下载失败，将公网 URL 切换为内网地址重试: {internal_url}"
            )
            try:
...
async def mineru_process(
    ...
    logger.info(
        f"保存初次结果为json文件成功，已将图片描述文本替换到[]里，文件路径: {json_file_path}"
    )
    ...
    logger.info(
        f"将json文件中的全部类目全部条目embedding做嵌入完成，共{len(results)}个条目，文件路径: {json_file_path}"
    )
    ...
    logger.info(
        "将json文件中的全部类目全部条目存入向量数据库完成，共{len(results)}个条目"
    )
    ...
    try:
        ...
        async with conn.transaction():
            await conn.execute(
                f"""UPDATE chunk_schema.documents SET markdown_public_url = '{md_file_public_url}', raw_file_public_url = '{raw_file_url_to_return}', name = '{raw_file_name}', upload_time = now() WHERE id = '{file_uuid}'"""
            )
        logger.info(
            f"用户{user_id}上传md文件到minio成功: {md_file_public_url},且将源文件的pdf格式上传到minio成功: {pdf_file_public_url},且更新数据库成功"
        )

        # 返回md文件的公网url
        return {
            "markdown_public_url": f"{config['server_components']['minio']['public_url_prefix']}/{bucket_name}/{user_id}/knowledgebase/{knowledge_base_id}/{file_uuid}/{file_uuid}.md",
            "pdf_file_public_url": f"{config['server_components']['minio']['public_url_prefix']}/{bucket_name}/{user_id}/knowledgebase/{knowledge_base_id}/{file_uuid}/{file_uuid}.pdf",
            "file_uuid": file_uuid,
        }
        ...
        await asyncio.to_thread(
            Path(file_path.with_suffix(".md")).unlink, missing_ok=True
        )
        ...
        await conn.close()
        logger.info(
            f"删除本地文件: {file_path} 和 {file_path.with_suffix('.md')} 和 {json_file_path}"
        )
...
if __name__ == "__main__":
    ...
    asyncio.run(
        mineru_process(
            "http://1.tcp.cpolar.cn:21729/publicfiles/xx.pdf",
            "xxx",
            "simple",
            "e4f7d0a3-fa32-4ca6-964e-01d02104844b",
        )
    )
