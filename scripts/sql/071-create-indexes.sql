...
-- 存储配置表索引
CREATE INDEX IF NOT EXISTS idx_storage_configs_platform_name ON chunk_schema.storage_configs(platform_name);
CREATE INDEX IF NOT EXISTS idx_storage_configs_platform_type ON chunk_schema.storage_configs(platform_type);
...
CREATE INDEX IF NOT EXISTS idx_storage_configs_platform_active ON chunk_schema.storage_configs(platform_name, is_active);
...
CREATE INDEX IF NOT EXISTS idx_files_storage_platform ON chunk_schema.files(storage_platform);
CREATE INDEX IF NOT EXISTS idx_files_storage_url ON chunk_schema.files(storage_url);
CREATE INDEX IF NOT EXISTS idx_files_internal_url ON chunk_schema.files(internal_url) WHERE internal_url IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_files_public_url ON chunk_schema.files(public_url) WHERE public_url IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_files_cdn_url ON chunk_schema.files(cdn_url) WHERE cdn_url IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_files_storage_config_platform ON chunk_schema.files(storage_config_id, storage_platform) WHERE storage_config_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_files_user_storage_platform ON chunk_schema.files(user_id, storage_platform);
...
CREATE INDEX IF NOT EXISTS idx_file_processing_logs_started_at ON chunk_schema.file_processing_logs(started_at);
...
CREATE INDEX IF NOT EXISTS idx_extracted_documents_quality ON chunk_schema.extracted_documents(extraction_quality_score) WHERE extraction_quality_score IS NOT NULL;

-- 全文搜索索引
CREATE INDEX IF NOT EXISTS idx_extracted_documents_fts ON chunk_schema.extracted_documents USING gin(to_tsvector('english', plain_text)) WHERE plain_text IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_extracted_documents_fts_jieba ON chunk_schema.extracted_documents USING gin(to_tsvector('jieba', plain_text)) WHERE plain_text IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_extracted_assets_storage_platform ON chunk_schema.extracted_assets(storage_platform);
CREATE INDEX IF NOT EXISTS idx_extracted_assets_storage_url ON chunk_schema.extracted_assets(storage_url);
CREATE INDEX IF NOT EXISTS idx_extracted_assets_internal_url ON chunk_schema.extracted_assets(internal_url) WHERE internal_url IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_extracted_assets_public_url ON chunk_schema.extracted_assets(public_url) WHERE public_url IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_extracted_assets_cdn_url ON chunk_schema.extracted_assets(cdn_url) WHERE cdn_url IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_extracted_assets_storage_platform_type ON chunk_schema.extracted_assets(storage_platform, asset_type);
...
CREATE INDEX IF NOT EXISTS idx_components_page_number ON chunk_schema.components(page_number) WHERE page_number IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_components_status ON chunk_schema.components(status) WHERE status IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_components_content_hash ON chunk_schema.components(content_hash) WHERE content_hash IS NOT NULL;
...
-- 全文搜索索引（基于简化后的组件表结构）
CREATE INDEX IF NOT EXISTS idx_components_content_fts ON chunk_schema.components USING gin(to_tsvector('english', content)) WHERE content IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_components_content_fts_jieba ON chunk_schema.components USING gin(to_tsvector('jieba', content)) WHERE content IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_components_type_status ON chunk_schema.components(component_type, status) WHERE status IS NOT NULL;

-- 文本块表索引（基于简化后的结构）
CREATE INDEX IF NOT EXISTS idx_chunks_component_id ON chunk_schema.chunks(component_id);
CREATE INDEX IF NOT EXISTS idx_chunks_content_hash ON chunk_schema.chunks(content_hash) WHERE content_hash IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_chunks_subtype ON chunk_schema.chunks(chunk_subtype) WHERE chunk_subtype IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_chunks_language ON chunk_schema.chunks(detected_language) WHERE detected_language IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_chunks_previous_chunk ON chunk_schema.chunks(previous_chunk_id) WHERE previous_chunk_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_chunks_next_chunk ON chunk_schema.chunks(next_chunk_id) WHERE next_chunk_id IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_chunks_text_fts_jieba ON chunk_schema.chunks USING gin(to_tsvector('jieba', text_content)) WHERE text_content IS NOT NULL;

-- 图片表索引（基于简化后的结构）
CREATE INDEX IF NOT EXISTS idx_photos_component_id ON chunk_schema.photos(component_id);
CREATE INDEX IF NOT EXISTS idx_photos_extracted_asset_id ON chunk_schema.photos(extracted_asset_id) WHERE extracted_asset_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_photos_subtype ON chunk_schema.photos(photo_subtype) WHERE photo_subtype IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_photos_detected_text ON chunk_schema.photos(detected_text) WHERE detected_text IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_components_document_type_status ON chunk_schema.components(document_id, component_type, status) WHERE status IS NOT NULL;
...
-- 内容质量分析
CREATE INDEX IF NOT EXISTS idx_extracted_documents_quality_range ON chunk_schema.extracted_documents(extraction_quality_score) WHERE extraction_quality_score IS NOT NULL;
