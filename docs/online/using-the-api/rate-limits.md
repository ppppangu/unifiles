...
当前 0.1 自部署 Server 不内置按请求次数或套餐分层的速率限制；反向代理、SaaS
部署或未来适配器可以返回 `429`，客户端会按响应头处理重试。不要把 SaaS 套餐数字
写进自部署应用的业务逻辑。

当前 Server 的资源限制来自环境变量：

| 限制 | 默认值 | 配置 |
|---|---:|---|
| 单文件大小 | 100 MiB | `UNIFILES_MAX_FILE_SIZE_BYTES` |
| 用户存储空间 | 10 GiB | `UNIFILES_STORAGE_LIMIT_BYTES` |
| 提取页数配额 | 100,000 | `UNIFILES_EXTRACTION_PAGES_LIMIT` |
| 知识库数量 | 1,000 | `UNIFILES_KNOWLEDGE_BASE_LIMIT` |

使用 `client.usage.stats()` 和 `client.usage.limits()` 查看当前部署返回的资源用量与配额。
